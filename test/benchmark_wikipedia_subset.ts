...
const heads = 8;
const alpha = 1e-5;
const benchmark = "wikipedia_subset";
...
async function benchmarkWikipediaSubset() {
  const vocabPath = path.join(__dirname, "..", "project", "generative-bot", "dataset", "generative_vocab.json");
  const datasetPath = path.join(__dirname, "..", "dataset", "wikipedia_belum_normalisasi.txt");
  ...
  if (samples.length === 0) {
    throw new Error("No samples were generated from the Wikipedia subset benchmark.");
  }
...
benchmarkWikipediaSubset().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
